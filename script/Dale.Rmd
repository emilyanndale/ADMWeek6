...
The measure we want to focus on is specificity. We want to avoid wasting resources (staff time, mailing fees, etc.), so finding true negatives is the most important factor. In this case, the model found true negatives at a rate of 97.2%, so we can say with 97% certainty that those customers shouldn't be contacted. More effort and resources can be spent on the other customers. 
